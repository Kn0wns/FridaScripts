...
let ourlib = "libxyz.so";
let p_pthread_create = Module.findExportByName("libc.so", "pthread_create");
let pthread_create = new NativeFunction(p_pthread_create, "int", ["pointer", "pointer", "pointer", "pointer"]);
Interceptor.replace(p_pthread_create, new NativeCallback(function (ptr0, ptr1, ptr2, ptr3) {
    ...
    if (gmn(ptr1) == ourlib) {
        let Mod = Module.findBaseAddress(ourlib)
...
let inet_atonPtr = Module.findExportByName("libc.so", "inet_aton");
let inet_aton = new NativeFunction(inet_atonPtr, 'int', ['pointer', 'pointer']);
Interceptor.replace(inet_atonPtr, new NativeCallback(function (addrs, structure) {
    let retval = inet_aton(addrs, structure);
    console.log("inet_aton : ", addrs.readCString())
    return retval;
}, 'int', ['pointer', 'pointer']))
let popenPtr = Module.findExportByName("libc.so", "popen");
let popen = new NativeFunction(popenPtr, 'pointer', ['pointer', 'pointer']);
Interceptor.replace(popenPtr, new NativeCallback(function (path, type) {
    let retval = popen(path, type);
    console.log("popen : ", path.readCString());
    return retval;
}, 'pointer', ['pointer', 'pointer']))
let symlinkPtr = Module.findExportByName("libc.so", "symlink");
let symlink = new NativeFunction(symlinkPtr, 'int', ['pointer', 'pointer']);
Interceptor.replace(symlinkPtr, new NativeCallback(function (target, path) {
    let retval = symlink(target, path);
    console.log("symlink: ", target.readCString(), path.readCString());
    return retval;
}, 'int', ['pointer', 'pointer']))
let symlinkatPtr = Module.findExportByName("libc.so", "symlinkat");
let symlinkat = new NativeFunction(symlinkatPtr, 'int', ['pointer', 'int', 'pointer']);
Interceptor.replace(symlinkatPtr, new NativeCallback(function (target, fd, path) {
    let retval = symlinkat(target, fd, path);
    console.log("symlinkat : ", target.readCString(), path.readCString());
    return retval;
}, 'int', ['pointer', 'int', 'pointer']))
let inet_addrPtr = Module.findExportByName("libc.so", "inet_addr");
let inet_addr = new NativeFunction(inet_addrPtr, 'int', ['int']);
Interceptor.replace(inet_addrPtr, new NativeCallback(function (path) {
    let retval = inet_addr(path);
    console.log("inet_addr : ", path.readCString())
    return retval;
}, 'int', ['int']))
let socketPtr = Module.findExportByName("libc.so", "socket");
let socket = new NativeFunction(socketPtr, 'int', ['int', 'int', 'int']);
Interceptor.replace(socketPtr, new NativeCallback(function (domain, type, proto) {
    let retval = socket(domain, type, proto);
    console.warn("socket  : ", domain, type, proto, "Return : ", retval)
    return retval;
}, 'int', ['int', 'int', 'int']))
let connectPtr = Module.findExportByName("libc.so", "connect");
let connect = new NativeFunction(connectPtr, 'int', ['int', 'pointer', 'int']);
Interceptor.replace(connectPtr, new NativeCallback(function (fd, addr, len) {
    let retval = connect(fd, addr, len);
    let family = addr.readU16();
    let port = addr.add(2).readU16();
    //port = ((port & 0xff) << 8) | (port >> 8);
    console.warn("Connect : ", family, "Port : ", port, "Return : ", retval);
    return retval;
}, 'int', ['int', 'pointer', 'int']))
let sendPtr = Module.findExportByName("libc.so", "send");
let send2 = new NativeFunction(sendPtr, 'int', ['int', 'pointer', 'int', 'int']);
Interceptor.replace(sendPtr, new NativeCallback(function (socksfd, msg, slen, flag, daddr, dlen) {
    let retval = send2(socksfd, msg, slen, flag);
    console.log("send : ", socksfd, msg.readCString(), slen, flag);
    return retval;
}, 'int', ['int', 'pointer', 'int', 'int']))
let sendtoPtr = Module.findExportByName("libc.so", "sendto");
let sendto = new NativeFunction(sendtoPtr, 'int', ['int', 'pointer', 'int', 'int', 'pointer', 'int']);
Interceptor.replace(sendtoPtr, new NativeCallback(function (socksfd, msg, slen, flag, daddr, dlen) {
    let retval = sendto(socksfd, msg, slen, flag, daddr, dlen);
...
let readPtr = Module.findExportByName("libc.so", "read");
let read = new NativeFunction(readPtr, 'int', ['int', 'pointer', "int"]);

//if process name not work correctly you can replace manually with your package name here 
let FakeMaps = "/data/data/" + ProName + "/maps";
let FOpenMaps = "/data/data/" + ProName + "/fmaps";
let FakeTask = "/data/data/" + ProName + "/task";
let FakeExE = "/data/data/" + ProName + "/exe";
let FakeMounts = "/data/data/" + ProName + "/mounts";
let FakeStatus = "/data/data/" + ProName + "/status";
let MapsFile = new File(FakeMaps, "w");
let TaskFile = new File(FakeTask, "w");
let ExEFile = new File(FakeExE, "w");
let FMapsFile = new File(FOpenMaps, "w");
let FMountFile = new File(FakeMounts, "w");
let StatusFile = new File(FakeStatus, "w");
let MapsBuffer = Memory.alloc(512);
let TaskBuffer = Memory.alloc(512);
let ExEBuffer = Memory.alloc(512);
let FopenBuffer = Memory.alloc(512);
let MountBuffer = Memory.alloc(512);
let StatusBuffer = Memory.alloc(512);
let Open64MapsBuffer = Memory.alloc(512);
Interceptor.replace(openPtr, new NativeCallback(function (pathname, flag) {
    let FD = open(pathname, flag);
    let ch = pathname.readCString();
    ...
    if (ch.indexOf("/proc/") >= 0 && ch.indexOf("mounts") >= 0) {
        ...
        let mountname = Memory.allocUtf8String(FakeMounts);
    ...
    /*
      if (ch.indexOf("/proc/") >=0 && ch.indexOf("status") >=0) {
         ...
         let PStatus = StatusBuffer.readCString();
         ...
         StatusFile.write(PStatus);
            ...
            let statusname = Memory.allocUtf8String(FakeStatus);
    ...
    if (ch.indexOf("/proc") >= 0 && ch.indexOf("exe") >= 0) {
        ...
        while (parseInt(read(FD, ExEBuffer, 512)) !== 0) {
            let buffer = ExEBuffer.readCString();
            //  console.warn(buffer)
        ...
        let filename3 = Memory.allocUtf8String(FakeExE);
...
let readlinkPtr = Module.findExportByName("libc.so", "readlink");
let readlink = new NativeFunction(readlinkPtr, 'int', ['pointer', 'pointer', 'int']);
Interceptor.replace(readlinkPtr, new NativeCallback(function (pathname, buffer, bufsize) {
    let retval = readlink(pathname, buffer, bufsize);
...
let readlinkatPtr = Module.findExportByName("libc.so", "readlinkat");
let readlinkat = new NativeFunction(readlinkatPtr, 'int', ['int', 'pointer', 'pointer', 'int']);
Interceptor.replace(readlinkatPtr, new NativeCallback(function (dirfd, pathname, buffer, bufsize) {
    let retval = readlinkat(dirfd, pathname, buffer, bufsize);
...
/*
let memcpyPtr = Module.findExportByName("libc.so", "memcpy");
let memcpy = new NativeFunction(memcpyPtr, 'pointer', ['pointer', 'pointer', 'int']);
Interceptor.replace(memcpyPtr, new NativeCallback(function(dest, src, len) {
    let retval = memcpy(dest, src, len);
    if(dest.readCString() != null && src.readCString() != null && (dest.readCString().indexOf("frida")>=0 || src.readCString().indexOf("frida")>=0) )
    {
        ...
        let buffer = dest.readCString();
        let buffer2 = src.readCString();
        buffer = buffer.replaceAll("re.frida.server","StaySafeStayHappy");                                                         
        buffer = buffer.replaceAll("frida-agent-64.so","StaySafeStayHappy");
        buffer = buffer.replaceAll("rida-agent-64.so","StaySafeStayHappy");
        buffer = buffer.replaceAll("agent-64.so","StaySafeStayHappy");        
